...
async def async_setup_entry(
    ...
    if isinstance(data[entry.entry_id], StackedScenes.Hub):
        ...
        for scene in hub.scenes:
            entities += [
                TransitionNumber(scene),
                DebounceTime(scene),
                ScenePriority(scene),
            ]

    elif isinstance(data[entry.entry_id], StackedScenes.Scene):
        scene = data[entry.entry_id]
        entities += [TransitionNumber(scene), DebounceTime(scene), ScenePriority(scene)]
...
class ScenePriority(RestoreNumber):
    """Time to wait after activating a scene switch until evaluating if the scene is still active."""
    ...
    _attr_name = "Scene Priority"
    _attr_entity_category = EntityCategory.CONFIG

    def __init__(self, scene: StackedScenes.Scene) -> None:
        """Initialize."""
        self._scene = scene
        self._name = f"{scene.name} Scene Priority"
        self._attr_unique_id = f"{scene.id}_scene_priority"
        ...
        self._scene.set_priority(scene.priority)
    ...
    def set_native_value(self, value: float) -> None:
        """Update the current value."""
        self._scene.set_priority(value)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if (last_state := await self.async_get_last_state()) and (
            ...
            if last_state.state not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                ...
                self._scene.set_priority(last_number_data.native_value)

    @property
    def native_value(self) -> float:
        """Return the entity value to represent the entity state."""
        return self._scene.priority
