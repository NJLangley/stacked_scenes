...
class ConfigFlow(config_entries.ConfigFlow, domain=PLATFORM):
    ...
    async def async_step_integration_discovery(
        ...
        unique_id = f"stacked_{discovery_info[CONF_SCENE_ENTITY_ID]}"
    ...
    async def async_step_select_external_scenes(self, user_input=None):
        ...
        if user_input is not None:
            ...
            unique_id = f"stacked_{self.configuration[CONF_SCENE_ENTITY_ID]}"
        ...
        try:
            hub = [
                entry
                for entry in self.hass.data[PLATFORM].values()
        ...
        excluded_entities += [
            entry.unique_id.replace("stacked_", "")
            for entry in self.hass.config_entries.async_entries(PLATFORM)
            if entry.unique_id
        ]
