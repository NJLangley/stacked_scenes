...
def setup_platform(
    ...
    hub = StackedScenes.Hub(hass, scene_path)

    # Add devices
    add_entities(StackedSceneSwitch(scene) for scene in hub.scenes)
...
async def async_setup_entry(
    ...
    _LOGGER.debug(
        "Setting up Stacked Scenes with data: %s and config_entry %s",
        data,
        entry,
    )
    entities = []
...
class StackedSceneSwitch(SwitchEntity):
    """Representation of an Awesome Light."""

    _attr_assumed_state = False
    _attr_has_entity_name = True
    _attr_name = "Stacked Scene"
    ...
    def __init__(self, scene) -> None:
        ...
        self._scene = scene
        self._is_on = False
        self._name = "Stacked Scene"
        self._icon = scene.icon
        self._attr_unique_id = f"stacked_{scene.id}"
